...
npx choir-cli config --relay https://choir-relay.<you>.workers.dev --name Bob

# join a session (the code comes from the host's /choir:share)
npx choir-cli join znx2fusf-zbxd
```
...
> Prefer a shorter command? `npm i -g choir-cli` once and it's just `choir join <code>`.

## Commands

| Command | What it does |
|---------|--------------|
| `choir-cli join <code>` | Join and watch/steer a shared session |
| `choir-cli take <code> --name <you>` | Accept a handoff (then run `/choir:take-handoff` in Claude Code) |
| `choir-cli config --relay <url> [--name <you>]` | Save your relay + display name |
